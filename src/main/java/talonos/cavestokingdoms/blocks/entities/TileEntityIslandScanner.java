...
public class TileEntityIslandScanner extends TileEntity 
{
    ...
    public void updateEntity() 
    {
		...
		if (lastScannedChunk == 0)
		{
			...
			//You have not started scanning yet.
			for (int x = -2; x<= 2; x++)
			{
				for (int y = 0; y<= 6; y++)
				{
					TileEntity hopefullyAMap = worldObj.getTileEntity(this.xCoord+x, this.yCoord+y, this.zCoord+7);
	            	if (hopefullyAMap instanceof TileEntityIslandMapper)
	            	{
	            		...
	            		worldObj.markBlockForUpdate(map.xCoord, map.yCoord, map.zCoord);
	            	}
	            	else
	            	{
	            	}
    	...
    	if (lastScannedChunk != -1)
    	{
    		...
    		for (int xInChunk = 0; xInChunk < 16; xInChunk+=2)
    		{
    			...
    			for (int zInChunk = 0; zInChunk < 16; zInChunk+=2)
    			{
                    ...
                    biomesForGeneration = this.worldObj.getWorldChunkManager().loadBlockGeneratorData(biomesForGeneration, x, z, 1, 1);
                    if ((biomesForGeneration != null) && (biomesForGeneration[0] != null)) 
                    {
                    	...
                    	if (lastScannedChunk == 0)
                    	{
                    		System.out.println("Chunk five: Scanning "+x+", "+z+": got a "+(getTaintAt(x,z)?"tainted ":"")+biomesForGeneration[0].biomeName);
                    	}
                    	...
                    	if (lastScannedChunk == 0)
                    	{
                    		System.out.println("Putting it at "+monitorXHeight+", "+monitorYHeight);
                    	}
                    	
                    	Set<TileEntityIslandMapper> toUpdate = new HashSet<TileEntityIslandMapper>();
                    	
                    	TileEntity hopefullyAMap = worldObj.getTileEntity(this.xCoord+monitorXHeight, this.yCoord+monitorYHeight, this.zCoord+7);
                    	if (hopefullyAMap instanceof TileEntityIslandMapper)
                    	{
                    		TileEntityIslandMapper map = (TileEntityIslandMapper) hopefullyAMap;
                    	...
                    	{
                    		System.out.println("Danger! Entity at "+(this.xCoord+monitorXHeight)+", "+(this.yCoord+monitorYHeight)+", "+(this.zCoord+7)+" is not a map, but instead a "+hopefullyAMap+"!");
                    	}
                    	
                    	for (TileEntityIslandMapper map : toUpdate)
                    	{
                    		worldObj.setBlockMetadataWithNotify(map.xCoord, map.yCoord, map.zCoord, worldObj.getBlockMetadata(map.xCoord, map.yCoord, map.zCoord)+1, 2);
                    	}
                    }
                    else
                    {
